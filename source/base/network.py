...
class NetworkClient(ExtendedTcpSocket):
    ...
    def connect_to_channel(self, channel_name, callback: callable):
        """
    ...
    def disconnect_from_channel(self, channel_name, callback: callable):
        """
    ...
    def _process(self, letter):
        ...
        self.channels[channel_name].received.emit(self, letter['content'])
    ...
    def send(self, channel_name, message=None):
        """
        Given a channel name and a message (optional) wraps them in one dict (a letter) and sends it.
