...
class ConfigParserExtended(configparser.ConfigParser):
    ...
    def get_int(self, section, option, default=0, int_min=-sys.maxsize + 1, int_max=sys.maxsize,
                even=True, odd=True, expected_values=None):
        nb_error = len(self.errors)
        try:
            retval = self.getint(section, option)
            if retval < int_min:
                self.errors.append(
                    'In file \'%s\' and section \'%s\' : Bad option \'%s\' should be smaller than %d (current:%d)'
                    % (self.file_name, section, option, int_min, retval))
            if retval > int_max:
                self.errors.append(
                    'In file \'%s\' and section \'%s\' : Bad option \'%s\' should be greater than %d (current:%d)'
                    % (self.file_name, section, option, int_max, retval))
            # TODO - int should not be expected to be both odd and even?
            if retval % 2 == 0 and not even:
                self.errors.append(
                    'In file \'%s\' and section \'%s\' : Bad option \'%s\' should be an odd number (current:%d)'
                    % (self.file_name, section, option, retval))
            if retval % 2 == 1 and not odd:
                self.errors.append(
                    'In file \'%s\' and section \'%s\' : Bad option \'%s\' should be an even number (current:%d)'
                    % (self.file_name, section, option, retval))
