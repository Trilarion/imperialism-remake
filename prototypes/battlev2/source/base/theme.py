...
class Theme:
    ...
    def __init__(self, name, description, coat_of_arms_graphics, flag_graphics, map_graphics,
        ...
        if not isinstance(coat_of_arms_graphics, str) or coat_of_arms_graphics == '' or  not os.path.isdir(coat_of_arms_graphics):
            raise ValueError('coat_of_arms_graphics must be a non empty string, corresponding to the path of a existing directory')
        if not isinstance(flag_graphics, str) or flag_graphics == '' or  not os.path.isdir(flag_graphics):
            raise ValueError('flag_graphics must be a non empty string, corresponding to the path of a existing directory')
        if not isinstance(map_graphics, str) or map_graphics == '' or  not os.path.isdir(map_graphics):
            raise ValueError('map_graphics must be a non empty string, corresponding to the path of a existing directory')
        if not isinstance(unit_graphics, str) or unit_graphics == '' or  not os.path.isdir(unit_graphics):
            raise ValueError('unit_graphics must be a non empty string, corresponding to the path of a existing directory')
        ...
        if not isinstance(autocombat_button, str) or autocombat_button == '' or  not os.path.exists(autocombat_button):
            raise ValueError('autocombat_button must be a non empty string, corresponding to the path of a existing file')
